...
@app.post("/api/analyze_hybrid", response_model=Dict[str, Any])
async def analyze_hybrid(audio: UploadFile = File(...), target_text: str = Form(...)) -> Dict[str, Any]:
    """Analyzes the pronunciation using a Hybrid approach."""
    ...
    try:
        ...
        # Run Hybrid analysis
        result = pronunciation_app.analyze_hybrid(file_path, target_text)
        return result
    except Exception as e:
        logger.error(f"Error during hybrid analysis: {str(e)}", exc_info=True)
